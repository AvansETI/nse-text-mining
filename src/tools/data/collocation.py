...
def plot_creater(dfset, question, positivityscore, negativityscore):
    size = [positivityscore, negativityscore]
    labels = 'Positive', 'Negative'
    total = positivityscore + negativityscore
    explode = (0.1, 0.1)

    plt.pie(size, labels=labels, autopct=lambda p: '{:.0f}'.format(p * total / 100), colors=['g', 'r'], shadow=True, startangle=90, explode=explode)
